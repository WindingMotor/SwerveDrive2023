...
public class SwerveGoToP extends CommandBase {
  ...
  // Command constructor
  public SwerveGoToP(SwerveSubsystem swerveSubsystem,Supplier<Double> headingFunction,
  
  double xSetpoint, double ySetpoint, double tSetpoint, boolean reset, Pose2d resetPose,
  double tolerance, double Pvalue, double divider){
  ...
  public void initialize() {
    ...
    vXController.setP(Pvalue);
    vYController.setP(Pvalue);
  ...
  public void execute(){
      ...
      turningSpeed = -thetaController.calculate(swerveSubsystem.getRobotDegrees(), tSetpoint) / divider;
